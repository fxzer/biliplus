...
<details>
  ...
  <ol>
    <li>
      <a href="#关于项目">关于项目</a>
      <ul>
        <li><a href="#基于原插件的改动">基于原插件的改动</a></li>
      </ul>
    </li>
    <li>
      <a href="#快速开始">快速开始</a>
...
本项目 Fork 自 [0xlau/biliplus](https://github.com/0xlau/biliplus)，主要改动：

* 新增主题跟随系统深浅色自动切换，切换时自动刷新页面并保留视频进度和滚动位置（默认开启）。
* 视频倍速改为三种模式单选：无 / 滑块无级变速 / 下拉选择变速（0.5x–5.0x，默认下拉）。
* 总开关、首页「换一换」回溯功能默认开启。
* 移除视频封面 AI 总结、封面观看人数、隐藏指定用户评论功能。
* 修复后台脚本 Receiving end does not exist 报错，功能未开启时不再重复请求接口。
